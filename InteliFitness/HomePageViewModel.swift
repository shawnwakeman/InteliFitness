...
//    }
    ...
    func hasIncompleteWorkout(exerciseModules: [WorkoutLogModel.ExersiseLogModule]) -> Bool {
        let filterExerciseModules =  exerciseModules.filter { !$0.isLast }
        for exercise in filterExerciseModules {
            // Assuming ExersiseSet array inside the module and it has an isComplete property
            // Check if the module is empty
            if exercise.setRows.isEmpty {
                return true
            }
        ...
        // If we've made it through all the modules and sets without returning, everything is complete
        return false
    }
    
    func setEdits(hours: Int, Minutes: Int, Name: String, WorkoutId: UUID) {
        homePageModel.setEditsForHistory(hours: hours, Minutes: Minutes, Name: Name, workoutId: WorkoutId)
    }
    ...
    func editMyWorkouts(workoutName: String, exersiseModules: [WorkoutLogModel.ExersiseLogModule], id: UUID) {
        homePageModel.editMyWorkouts(workoutName: workoutName, exerciseModules: exersiseModules, id: id)
    }
//    func addToexercise
...
//    }
    ...
    func formatDate(_ date: Date) -> String {
        ...
        dateFormatter.dateFormat = "MMMM, EEEE dd"
        return dateFormatter.string(from: date)
    }

    func formatDate2(_ date: Date) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "MMMM, EEEE dd, 'at' h:mm a"
        return dateFormatter.string(from: date)
    }
