                    ...
                    ZStack {
                        HStack {
                            ...
                            TextField("", text: $workoutLogViewModel.workoutName, prompt: Text("").foregroundColor(Color("GrayFontOne")))
                                    ...
                                    .scaledToFit()
                                    .onTapGesture {
                                        if workoutLogViewModel.workoutName == "Workout Name" {
                    ...
                    ForEach(workoutLogViewModel.exersiseModules){ workoutModule in
                        if workoutLogViewModel.exersiseModules.firstIndex(where: { $0.id == workoutModule.id }) != nil {
                            if workoutModule.isLast == false {
                                ...
                                ExersiseLogModule(workoutLogViewModel: workoutLogViewModel, homePageViewModel: homePageVeiwModel, timeViewModel: timeViewModel, blocked: $blocked, parentModuleID: exerciseID, moduleUUID: workoutModule.id, isLive: true)
                                    .transition(.opacity)
                            }
                        }
...
//                    .scaleEffect(workoutLogViewModel.getPopUp(popUpId: "TitlePagePopUp").RPEpopUpState ? 1 : 0.5, anchor: .top)
            ...
            Group {
                ...
                NamePopUP( viewModel: workoutLogViewModel, timeViewModel: timeViewModel, isForCreator: false)
    ...
    //                .position(x: getScreenBounds().width / 2, y: getScreenBounds().height * 0.2)
            ...
            Group {
                ...
                VisualEffectView(effect: UIBlurEffect(style: .dark))
                    ...
                    .scaleEffect(1.3)
                FinishPopUp(homePageViewModel: homePageVeiwModel, timeViewModel: timeViewModel, viewModel: workoutLogViewModel)
                ...
                PausePopUp(viewModel: workoutLogViewModel, timeViewModel: timeViewModel)
                    .opacity(workoutLogViewModel.getPopUp(popUpId: "PausePopUp").RPEpopUpState ? 1 : 0)
            ...
            ZStack {
                VisualEffectView(effect: UIBlurEffect(style: .dark))
                    .edgesIgnoringSafeArea(.all)
                    .opacity(workoutLogViewModel.displayingExerciseViewOnWorkout ? 1 : 0)
                    .offset(y: getScreenBounds().height * -0.07)
                ExercisePage(viewModel: homePageVeiwModel, showingExrcisePage: $workoutLogViewModel.displayingExerciseViewOnWorkout, ignoringOffset: true)
                    .position(x: getScreenBounds().width/2, y: workoutLogViewModel.displayingExerciseViewOnWorkout ? getScreenBounds().height * 0.4 : getScreenBounds().height * 1.3)
        ...
        .onChange(of: homePageVeiwModel.ongoingWorkout) { newValume in
            if workoutLogViewModel.workoutName == "" {
            ...
            VStack {
                ...
                if !isForCreator {
                    ...
                    Button {
                        ...
                        timeViewModel.setTimeStep(step: 0)
                        withAnimation(.spring()) {
...
struct ElapsedTime : View {
    ...
    var body: some View{
        ...
        .onReceive(timeViewModel.workoutTime.time) { (_) in
            
            if timeViewModel.workoutTime.timeRunning{
...
struct WorkoutTimer : View {
    ...
    var body: some View{
        ...
        .onReceive(timeViewModel.workoutTime.time) { (_) in
            ...
            if timeViewModel.restTime.timeRunning{
        ...
        .onChange(of: timeViewModel.restTime.timeElapsed) { timeElapsed in
    ...
    var body: some View {
        VStack(spacing: -2){
            if workoutLogViewModel.exerciseModule(at: parentModuleID) != nil {
                ...
                LogModuleHeader(viewModel: workoutLogViewModel, blocked: $blocked, parentModuleID: parentModuleID, moduleUUID: moduleUUID, homePageViewModel: homePageViewModel, timeViewModel: timeViewModel, isForCreator: !isLive)
               
                if workoutLogViewModel.exersiseModules[parentModuleID].displayingNotes {

                    NotesModule(viewModel: workoutLogViewModel, homePageModelViewModel: homePageViewModel, parentModuleID: parentModuleID)
                }
                CoreLogModule(viewModel: workoutLogViewModel, ModuleID: parentModuleID, moduleUUID: moduleUUID, isLive: isLive, homePageViewModel: homePageViewModel, timeViewModel: timeViewModel)
            }
            ...
            HStack(alignment: .bottom){
                ...
                Button {HapticManager.instance.impact(style: .rigid)
                    ...
                    if isForCreator {
                        ...
                        homePageViewModel.setCurrentExercise(exercise: exercise)
                        withAnimation(.spring()) {
                            viewModel.displayingExerciseViewOnWorkout.toggle()
                        }
                    }
                ...
                ZStack{
                    ...
                    Button(action: {
                        withAnimation(.spring()) {
                            ...
                            timeViewModel.setTimePreset(time: viewModel.exersiseModules[parentModuleID].restTime)
                        }}, label: {
                            RoundedRectangle(cornerRadius: 3)
                                  .stroke(Color("BorderGray"), lineWidth: borderWeight)
                            .frame(width: getScreenBounds().width * 0.09, height: getScreenBounds().height * 0.03)})
...
struct NotesModule: View{
    @ObservedObject var viewModel: WorkoutLogViewModel
    @ObservedObject var homePageModelViewModel: HomePageViewModel
...
//                        let _ = print(module.moduleType)
                        ...
                        if module.moduleType == WorkoutLogModel.moduleType.reps {
                            HeaderRepsOnly(viewModel: viewModel, parentModuleID: ModuleID, TitleText: "Reps")
                        }
                            ...
                            HeaderRepsOnly(viewModel: viewModel, parentModuleID: ModuleID, TitleText: "Time")
                        }
                        else if module.moduleType == WorkoutLogModel.moduleType.cardio {
                            Header(viewModel: viewModel, parentModuleID: ModuleID, text1: "Mi", text2: "Time")
                        }
                        else {
                            Header(viewModel: viewModel, parentModuleID: ModuleID)
                        }
                    
                        ContentGrid(viewModel: viewModel, ModuleID: ModuleID, moduleUUID: moduleUUID, isLive: isLive, homePageViewModel: homePageViewModel, timeViewModel :timeViewModel)
                    } else {
                        EmptyView()
                    }
                        ...
                        if viewModel.lastRowUsed != 100 {
                            ...
                            if reps != 0 && weight != 0 {
                                ...
                                if viewModel.exersiseModules[viewModel.lastModuleUsed].setRows[viewModel.lastRowUsed].prevouslyChecked == false {
                                    timeViewModel.restAddToTime(step: 1, time: viewModel.exersiseModules[popUp.popUpExersiseModuleIndex].restTime)
                                    timeViewModel.restAddToTime(step: 1, time: timeViewModel.restTime.timePreset)
                            ...
                            if workoutModule.moduleType == WorkoutLogModel.moduleType.reps && reps != 0 {
                                let exerciseID = viewModel.getUUIDindex(index: workoutModule.id)
                                withAnimation(.spring()) {
                                    viewModel.toggleCompletedSet(ExersiseModuleID: exerciseID, RowID: workoutModule.setRows[viewModel.lastRowUsed].id, customValue: true)
                                }
    ...
    var body: some View {
        if viewModel.exerciseModule(at: ModuleID) != nil {
            ...
            let exercise = homePageViewModel.exersises[module.ExersiseID]
                ...
                ForEach(module.setRows) { row in
                   
                    if isLive {
                        let prevoiseString = viewModel.getPreviousSetString(for: row, in: exercise)
                       
                        ZStack {
                            if module.moduleType == WorkoutLogModel.moduleType.reps {
                           
                                WorkoutSetRowViewRepsOnly(viewModel: viewModel, timeViewModel: timeViewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString)
                            } else if (module.moduleType == WorkoutLogModel.moduleType.cardio) {
                                WorkoutSetRowViewCardio(viewModel: viewModel, timeViewModel: timeViewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString)
                                
                            } else if (module.moduleType == WorkoutLogModel.moduleType.duration) {
                                WorkoutSetRowViewDuration(viewModel: viewModel, timeViewModel: timeViewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString)
                          
                            } else {
                                
                                WorkoutSetRowView(viewModel: viewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString, timeViewModel: timeViewModel)
                                
                            }
                        }
                        .onAppear {
                            viewModel.SetPlaceHolders(for: row, in: exercise, moduleId: ModuleID)
                        }
                        ...
                        if module.moduleType == WorkoutLogModel.moduleType.reps {
                            WorkoutSetRowViewForCreatorRepsOnly(viewModel: viewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString)
                        } else if (module.moduleType == WorkoutLogModel.moduleType.cardio) {
                            WorkoutSetRowViewForCreatorCardio(viewModel: viewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString)
                        } else if (module.moduleType == WorkoutLogModel.moduleType.duration) {
                            WorkoutSetRowViewForCreatorDuration(viewModel: viewModel, rowObject: row, moduleID: ModuleID, moduleUUID: moduleUUID, previous: prevoiseString)
                        } else {
...
struct Header: View {
    ...
    var text1: String = "Lbs"
    var text2: String = "Reps"
    var body: some View{
        ZStack{
            Rectangle()
            ...
            VStack{
                HStack{
                    ...
                    TextHelvetica(content: plusOrMinus + text1, size: 20)
                        .foregroundColor(Color("WhiteFontOne"))
                    
                    Spacer()

                    TextHelvetica(content: text2, size: 20)
                        .padding(.trailing)
                        .foregroundColor(Color("WhiteFontOne"))
...
enum RecurringOption2 {
    case none, daily, weekly
}

func scheduleNotificationForReminder(id: Int, title: String, body: String, date: Date, offset: Int, recurringOption: RecurringOption2) {
    let notificationCenter = UNUserNotificationCenter.current()
    ...
    // Subtract offset minutes from the date
    guard let newDate = Calendar.current.date(byAdding: .minute, value: -offset, to: date) else {
        return
    }

    // Calculate the time interval from now until the new date
    ...
    // Use a unique identifier for the request
    let requestIdentifier = "yourAppName.reminder.notification.\(id)"
    ...
    if recurringOption == .none {
        ...
        // Extract the date components from the new date
